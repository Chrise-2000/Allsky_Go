...
def run_script():
    ...
    subprocess.run(["python", "gfd_buzzer.py"], check=True)
    subprocess.run(["/bin/bash", "/home/pi/allsky/scripts/generateForDay.sh", input_dir], check=True)
    subprocess.run(["python", "done_buzzer.py"], check=True)
...
def handle_press():
    ...
    if press_duration >= 2:  # Long press (≥2s) → Shutdown
        subprocess.run(["python", "shutdown_buzzer.py"], check=True)
        subprocess.run(["sudo", "shutdown", "-h", "now"], check=True)
    else:  # Short press → Run WPS connection script
        subprocess.run(["python", "wps_buzzer.py"], check=True)
        subprocess.run(["python", "wps_connection.py"], check=True)
        subprocess.run(["python", "done_buzzer.py"], check=True)

button_a.when_pressed = run_script  # Run script when Button A is pressed
button_b.when_pressed = handle_press  # Button B triggers action based on press duration
